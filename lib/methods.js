  ...
  indexSelected: function(index, gameId){
    ...
    if(currentUserId == oldGame.mainGame.turn){
      ...
      if(currentUserId == oldGame.userId){
        var board = oldGame.mainGame.userBoard;
        board[index] = {
          number: board[index].number,
          selected: true
        };
        return Games.update(gameId, {
          $set: { 'mainGame.userBoard': board, 'mainGame.turn': oldGame.opponentId }
        });
      }else{
        var board = oldGame.mainGame.opponentBoard;
        board[index] = {
          number: board[index].number,
          selected: true
        };
        return Games.update(gameId, {
          $set: { 'mainGame.opponentBoard': board, 'mainGame.turn': oldGame.userId }
        });
      }
    }else{
      return Error();
